const misingProductsList = document.getElementById("products__mising_list");

const arrowForMisingProducts = document.querySelector(
  ".products__mising_arrow"
);

const createQuantityMisingProducts = () => {
  const quantityMisingProducts = document.getElementById(
    "products__mising_quantity"
  );
  return (quantityMisingProducts.innerHTML = `Отсутствуют · ${
    products.filter((product) => product.isStockAvailability === false).length
  } товара`);
};

arrowForMisingProducts.addEventListener("click", (e) => {
  arrowForMisingProducts.classList.toggle("close");
  misingProductsList.classList.toggle("close");
});

const createMisingProductsList = () => {
  return (misingProductsList.innerHTML = products
    .filter((product) => product.isStockAvailability === false)
    .map((product) => {
      const { id, image, title, characteristic, isFavourite } = product;

      return `<article class="commodity">
        ...
        <div class="products__mising_functions__additional">
